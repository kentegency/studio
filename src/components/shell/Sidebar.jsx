...
const RoomsIcon   = () => <svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
...
function SbIcon({ Icon, active, onClick, dot }) {
  return (
    <button className={`sbi ${active ? 'on' : ''}`} onClick={onClick} data-hover>
      <Icon />
      {dot && <span className="sbi-dot" />}
    </button>
  )
}
...
export default function Sidebar() {
  ...
  return (
    <aside className="sidebar">
      ...
      <SbIcon Icon={ProjIcon}    active={active==='projects'} onClick={() => click('projects')} />
      <SbIcon Icon={RoomsIcon}   active={active==='rooms'}    onClick={() => click('rooms')} />
      <SbIcon Icon={BellIcon}    active={active==='notifs'}   onClick={() => click('notifs')} dot />
      <SbIcon Icon={SearchIcon}  active={active==='search'}   onClick={() => click('search')} />
      <SbIcon Icon={SketchIcon}  active={active==='sketch'}   onClick={() => click('sketch',  () => openOverlay('sketch'))} />
      <SbIcon Icon={CompareIcon} active={active==='compare'}  onClick={() => click('compare', () => openOverlay('compare'))} />

      <div className="sb-spacer" />

      <SbIcon Icon={StageIcon}   active={active==='stage'}    onClick={() => click('stage', () => { openOverlay('stage'); showToast('Lights up.') })} />
      <SbIcon Icon={SettingsIcon}active={active==='settings'} onClick={() => click('settings')} />
    </aside>
  )
}
